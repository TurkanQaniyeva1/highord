...
let filterHeight = users.filter(users => users.height > 150);
console.log(filterHeight);

//2

function getPerson(firstName) {
    let person = users.find(users => users.firstName === firstName);
    if (person) {
        console.log(person);
    } else {
        console.log("Tapilmadi " + firstName);
    }
}
...
const getAge = () => {
    let usersAge = users.reduce((total, num) => total + num.age, 0);
    let averageAge = usersAge / users.length;
    console.log("Toplam yas:", usersAge);
    console.log("Ortaq yas:", averageAge);
}
...
const sortAge = () => {
    ...
    console.log("Artan", users);
};
sortAge(); 

const sortAge2 = () => {
    users.sort((a, b) => b.age - a.age); 
    console.log("Azalan", users);
};
sortAge2();  
...
const developers = () => {
    let webDev = users
        .map(user => user.company.title === "Web Developer" ? user : null) 
